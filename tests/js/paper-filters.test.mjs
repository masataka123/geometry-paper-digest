...
const baseState = { q: "", author: "all", tag: "all", topic: "all", year: "all", sort: "arxiv-newest" };
...
test("round trips valid query state", () => {
  const state = { q: "Chern 類", author: "alice", tag: "chern", topic: "ag", year: "2025", sort: "site-newest" };
...
test("invalid query values fall back safely", () => {
  assert.deepEqual(readFilterState("?q=%20test%20&author=nope&topic=nope&year=x&sort=x", ["all", "ag"], ["all", "2025"]), { q: "test", author: "all", tag: "all", topic: "all", year: "all", sort: "arxiv-newest" });
});
test("combines author with q, topic, year, and sort", () => {
  const state = { q: "chern", author: "alice", tag: "chern", topic: "ag", year: "2025", sort: "site-newest" };
...
test("combines Pagefind matches with topic, year, and sort", () => {
  ...
  assert.deepEqual(filterAndSortPapers(papers, { q: "x", author: "all", tag: "all", topic: "ag", year: "2025", sort: "site-newest" }, matches).map((paper) => paper.pathname), ["/ag/"]);
