...
- 「イントロダクションの日本語要約」は原則5〜10段落（短い原文は水増ししない）で、背景、問題、既存研究、未解決点、着想、主要結果、新規性、応用・課題の流れを独自に整理する。逐語訳にしない。
- 証明済みの主張、予想、既知の結果、動機、将来課題、記事作成者の説明を区別する。
- 「論文の主要ポイント」はIntroductionから確認できる3〜7項目とする。厳密な仮定が不明なら「Introductionでは概略として次のように述べられている」と明記し、推測しない。
- 「論文の構成」はIntroductionに説明がある場合だけ記載し、なければ節ごと省略できる。
- Introductionの理解に不可欠な数式だけをAbstractまたはIntroductionと照合して掲載する。区切りは $ ... $ と別行の $$ ... $$ のみとし、LaTeXコマンドを不要に二重エスケープしない。
- 標準テンプレートに「要約」「背景と問題設定」「主結果」「主定理」「証明の見取り図」「研究上の位置づけ」「独立照合メモ」「Introductionの逐語訳」を独立した節として追加しない。必要な内容は下記の該当節へ統合する。
-->
...
<!-- Introductionに論文構成の説明がない場合は、この節を省略する。 -->
